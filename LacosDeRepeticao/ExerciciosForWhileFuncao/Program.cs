...
namespace ExerciciosForWhileFuncao
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("Escreva a palavra de busca : ");
            var wordCount = FindAnyLetter(Console.ReadLine());
        ...
        {
            // passando o conteudo para uma variável para que seja mais rápido e o computador não precisar ir até o método para ver o texto
            var contentToFind = TextContent();
            // passando o length para uma faviável ara o computador não precisar contar o length a cada laço for
...
";
        ...
        private static int FindAnyLetter (string wordFind)
        {

            // passando o conteudo para uma variável para que seja mais rápido e o computador não precisar ir até o método para ver o texto
            var contentToFind = TextContent();
            // passando o length para uma faviável ara o computador não precisar contar o length a cada laço for
            var lengthText = contentToFind.Length - (wordFind.Length -1);
            var contWordFind = 0;
            ...
            {
                var compareWord = string.Empty;
                for (int n = 0 ; n < wordFind.Length; n++)
                {
                    compareWord += contentToFind[i + n].ToString();
                }
